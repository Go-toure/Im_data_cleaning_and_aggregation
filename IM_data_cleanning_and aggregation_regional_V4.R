...
# Corrected version:
# - fixes invalid back reference regex error
# - uses Algeria-specific U6 HH columns only for form 8587
# - uses U5 HH columns for all other countries
# ============================================================
...
# literal matching helper (NOT regex-based)
...
# build literal HH candidate names
...
create_summary_columns <- function(df, file_name) {
  ...
  # Use U6 only for Algeria 8587; U5 for all others
  if (is_algeria_8587) {
    present_cols <- unique(c(
      names(df)[names(df) %in% sprintf("HH[%s]/Total_U6_Present_HH", 1:10)]
    ))
    fm_cols <- unique(c(
      names(df)[names(df) %in% sprintf("HH[%s]/U6_Vac_FM_HH", 1:10)]
    ))
  } else {
    present_cols <- unique(c(
      names(df)[names(df) %in% sprintf("HH[%s]/Total_U5_Present_HH", 1:10)]
    ))
    fm_cols <- unique(c(
      names(df)[names(df) %in% sprintf("HH[%s]/U5_Vac_FM_HH", 1:10)]
    ))
  }
...
process_im_file <- function(input_file, output_folder, qc_output_folder, lookup_table) {
  ...
  # choose HH patterns by form id
  active_hh_patterns <- if (identical(file_name, ALGERIA_IM_FORM_ID)) hh_patterns_algeria else hh_patterns_standard
